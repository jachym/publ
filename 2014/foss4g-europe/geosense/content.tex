...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}\begin{center}
    \includegraphics<1>[width=0.4\textwidth]{imgs/logoGEOSENSE_rgb2.png}
    \includegraphics<2>[width=0.7\textwidth]{imgs/unnamed4.jpg}
\note<1>{
    I'm here talking as member of the development team at Geosense company.
    Geosense is czech company with international ambitions, which was founded in
    2009. We used to say, that we are doing information systems with strong
    spatial informations. 
}
\note<2>{
    It was founded by Vlada Čapek and Jan Zvonik - as you can see, they are nice
    guys - and the development team, which was formed about year ago, has
    started to work on something, we call the new geoportal. Why new: because
    it's supposed to replace the old product. It has of course server and client
    components and today, I'm going to present the way, how we came to the
...
\note<1>{Back to my self or to my jurney:
    I switched from desktop to web developemnt during 2007, and it was
...
\note<3>{
    This is the key concept of the application: user can make a choice between
    data-centrick and map-centrick view on the data. Either he or she focuses on
    the table, applies filters, search in the table. Then he switches to map,
    can filter the or search the data again...
...
\only<3>{
\includegraphics[width=0.7\textwidth]{imgs/Power_connector_Legrand_32A.jpg}\\
    API
}
...
\begin{frame}\begin{center}
    \only<1-2>{
        \begin{itemize}
            \item Setup dev environment
            \item Moving dev environment
            \item Node.js builder
            \item Jquery
...
\includegraphics[width=0.7\textwidth]<3>{imgs/LogoMess_vectorized_72dpi.jpg}
\note<3>{
...
\begin{frame}\begin{center}
    \begin{itemize}
        \item Google Closure library
        \item OpenLayers 3
        \item Bootstrap (NO JQuery)
        \item Python build script
        \item Bower
        \item CasperJS
        \item Jenkins
    \end{itemize}
...
\note<1>{
    ...
    google closure library, we are using boostrap for some gui parts but without
    jquery components, casperjs for unit testing and integration testing and
    jenkins for automatic building process. For solving automatic dependencies,
    we use bower.
}
\end{center}\end{frame}
...
\begin{frame}\begin{center}
    ...
    \url{http://dev.geosense.cz/gp2p/master/examples/gp2.html}\\
    \url{http://dev.geosense.cz/gp2p/rc/examples/gp2.html}\\
\end{center}\end{frame}
